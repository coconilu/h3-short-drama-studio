...
function AgentReview({ run, provider, busy, onClose, onAction }: {
  ...
  return <div className="modal-backdrop planning-modal-backdrop"><section className="planning-history agent-review" role="dialog" aria-modal="true" aria-label="Agent 提案差异">
    ...
    <div className="agent-review-meta"><span>输入哈希 <code>{run.input_hash.slice(0, 12)}</code></span><span>目标修订 {run.base_revisions.map((item) => `${item.entity_type} R${item.revision}`).join(' · ') || '新建内容'}</span><span title={run.executable_path}>运行快照 {run.provider_adapter} · {run.model || '默认模型'} · {run.timeout_seconds}s · <code>{run.executable_fingerprint.slice(0, 12)}</code></span></div>
