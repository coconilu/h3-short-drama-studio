...
export function SettingsPage({ settings, health, onSave, onTest, onRestartApi }: {
  ...
  const probeAgent = async (providerId: string) => {
    ...
    try {
      ...
      setProviders((items) => items.map((item) => item.id === providerId ? { ...item, callable: false, probe_state: 'unavailable', last_error: message, action_hint: `探测失败：${message}` } : item))
    } finally { setProbing('') }
  }
  ...
  return <main className="global-page settings-page">
    ...
    <form onSubmit={submit} className="settings-form">
      ...
      <section><div className="settings-section-copy"><Bot size={18} /><div><h2>本地 Agent</h2><p>分别配置并探测 Codex/Kimi CLI；命令、模型和超时会记录，但不会存储第三方密钥。</p></div></div><div className="agent-provider-grid">{providers.map((provider) => <article className={provider.callable ? 'online' : ''} key={provider.id}><header><span><i />{provider.label}</span><strong>{provider.callable ? '可调用' : provider.installed ? '调用异常' : '未安装'}</strong></header><p>{provider.action_hint}</p><div className="agent-provider-fields"><label>可执行文件<input value={provider.executable_path || ''} onChange={(event) => updateProviderDraft(provider.id, { executable_path: event.target.value })} placeholder={provider.adapter} /></label><label>模型（可选）<input value={provider.model} onChange={(event) => updateProviderDraft(provider.id, { model: event.target.value })} placeholder="使用 CLI 默认模型" /></label><label>超时秒数<input type="number" min="1" max="3600" value={provider.timeout_seconds} onChange={(event) => updateProviderDraft(provider.id, { timeout_seconds: Number(event.target.value) })} /></label><label className="switch-field"><input type="checkbox" checked={provider.enabled} onChange={(event) => updateProviderDraft(provider.id, { enabled: event.target.checked })} /><span>启用</span></label></div><small>{provider.version || provider.last_error || '尚未探测'}</small><footer><code>{provider.adapter} · {provider.capabilities.length} 层合同</code><div><button type="button" className="button secondary" disabled={probing === provider.id} onClick={() => probeAgent(provider.id)}>{probing === provider.id ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />}探测</button><button type="button" className="button secondary" disabled={savingProvider === provider.id} onClick={() => saveProvider(provider)}>{savingProvider === provider.id ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />}保存并探测</button></div></footer></article>)}{!providers.length && <div className="agent-provider-empty">Agent 状态暂时无法读取；不影响人工编辑。</div>}</div></section>
