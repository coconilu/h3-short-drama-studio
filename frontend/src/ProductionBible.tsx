...
export function ProductionBible({ projectId, assets, setNotice }: {
  ...
  const derivedSelected = draft?.source_type === 'creative_character'
  ...
  return <main className="bible-page">
    ...
    <section className="bible-workspace">
      ...
      <section className="bible-list">
        ...
        {visibleEntries.map((entry) => <button className={selectedId === entry.id ? 'active' : ''} key={entry.id} onClick={() => setSelectedId(entry.id)}><span className={`bible-status ${entry.status}`}><i />{entry.status === 'locked' ? '已锁定' : '草稿'}</span><strong>{entry.name}</strong><p>{entry.summary || typeMeta[entry.entry_type].hint}</p><footer><span>R{entry.revision}</span><span>{entry.source_type === 'creative_character' ? `角色卡 R${entry.source_revision}` : `${entry.assets.length} 素材`}</span><span>{entry.apply_globally ? '全局' : `${entry.shots.length} 镜头`}</span><ChevronRight size={14} /></footer></button>)}
      ...
      {draft ? <section className="bible-editor">
        <div className="bible-editor-head"><div><span>{typeMeta[draft.entry_type].label} · R{draft.revision}{draft.source_type === 'creative_character' ? ` · 来源：角色卡 ${draft.source_id} R${draft.source_revision}` : ' · 来源：人工维护'}</span><strong>{draft.name}</strong></div><div><button className="icon-action" title="查看版本" onClick={async () => setVersions(await api(`/api/bible/entries/${draft.id}/versions`))}><BookMarked size={17} /></button><button className="button secondary" disabled={busy || draft.source_type === 'creative_character'} onClick={save}><Save size={15} />保存修订</button><button className="button primary" disabled={busy || draft.status === 'locked' || draft.source_type === 'creative_character'} onClick={() => mutate(() => api(`/api/bible/entries/${draft.id}/lock`, { method: 'POST', body: JSON.stringify({ base_revision: draft.revision }) }), `“${draft.name}”已锁定，可供后续提示词编译`)}><LockKeyhole size={15} />{draft.status === 'locked' ? '已锁定' : '锁定版本'}</button></div></div>
        {draft.source_type === 'creative_character' && <p className="bible-source-notice"><ShieldCheck size={14} />此条目由已批准角色卡自动锁定；请回到“创作规划”修改视觉或声音规则并重新批准。</p>}
        <div className="bible-form">
          <label>条目名称<input disabled={derivedSelected} value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label>
          <label>一句话识别点<input disabled={derivedSelected} value={draft.summary} onChange={(event) => setDraft({ ...draft, summary: event.target.value })} placeholder="让团队一眼确认这是同一个对象" /></label>
          <label className="wide">标准设定<textarea disabled={derivedSelected} rows={4} value={draft.canonical_description} onChange={(event) => setDraft({ ...draft, canonical_description: event.target.value })} placeholder="稳定不变的身份、外形、空间、材质或声音事实；不要写本镜头动作" /></label>
          <label className="wide prompt-field">H3 提示词片段<textarea disabled={derivedSelected} rows={4} value={draft.prompt_fragment} onChange={(event) => setDraft({ ...draft, prompt_fragment: event.target.value })} placeholder="后续会由提示词编译器按镜头组合；建议使用清晰、可观察的英文描述" /></label>
          <label>负面约束<textarea disabled={derivedSelected} rows={3} value={draft.negative_prompt} onChange={(event) => setDraft({ ...draft, negative_prompt: event.target.value })} placeholder="不能出现的变体、服装、材质、声音等" /></label>
          <label>连续性规则<textarea disabled={derivedSelected} rows={3} value={draft.continuity_rules} onChange={(event) => setDraft({ ...draft, continuity_rules: event.target.value })} placeholder="跨镜头必须保持的状态，以及允许变化的条件" /></label>
          <label className="bible-global"><input disabled={derivedSelected} type="checkbox" checked={draft.apply_globally} onChange={(event) => setDraft({ ...draft, apply_globally: event.target.checked })} /><span><strong>应用到全片</strong><small>适合统一摄影风格、全片声音基调；角色与场景通常按镜头绑定。</small></span></label>
        </div>

        <section className="bible-bindings"><header><div><Image size={16} /><span><strong>真实参考素材</strong><small>仅受管素材可进入生产圣经；绑定变化会生成新修订并回到草稿。</small></span></div><div><select disabled={derivedSelected} aria-label="选择圣经参考素材" value={assetChoice} onChange={(event) => setAssetChoice(event.target.value)}><option value="">选择受管素材</option>{bindableAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select><button className="button secondary" disabled={derivedSelected || !assetChoice || busy} onClick={linkAsset}>绑定</button></div></header><div className="bible-media-list">{selected?.assets.map((asset) => <article key={asset.id}><MediaThumb asset={asset} /><span><strong>{asset.name}</strong><small>{asset.kind} · {asset.media_type}</small></span><button title="解除素材绑定" disabled={derivedSelected || busy} onClick={() => mutate(() => api(`/api/bible/entries/${selected.id}/assets/unlink`, { method: 'POST', body: JSON.stringify({ base_revision: selected.revision, asset_id: asset.id, role: asset.bible_role }) }), '已解除真实素材绑定')}><Unlink size={14} /></button></article>)}{!selected?.assets.length && <p>当前条目是文本设定；可以锁定，但不会为 Ref2VA 提供参考文件。</p>}</div></section>

        <section className="bible-shot-links"><header><div><ShieldCheck size={16} /><span><strong>镜头连续性覆盖</strong><small>{draft.apply_globally ? '此条目已全片生效，无需逐镜重复绑定。' : '明确哪些镜头需要这个对象，P12 才能按镜头编译。'}</small></span></div></header><div>{workspace.shots.map((shot) => { const linked = Boolean(selected?.shots.some((item) => item.shot_id === shot.id)); return <button key={shot.id} disabled={derivedSelected || busy || draft.apply_globally} className={linked || draft.apply_globally ? 'linked' : ''} onClick={() => toggleShot(shot.id, linked)}><span>{linked || draft.apply_globally ? <Check size={13} /> : null}</span><strong>{shortShotId(shot.id)}</strong><small>{shot.title}</small></button> })}</div></section>

        <footer className="bible-editor-footer"><span><ShieldCheck size={14} />每次保存、绑定和锁定都有不可变快照；旧页面提交会因修订号冲突而被拒绝。</span>{draft.source_type !== 'creative_character' && (archiveConfirm ? <div><small>归档会停止后续编译，但保留版本记录。</small><button className="button secondary" onClick={() => setArchiveConfirm(false)}>取消</button><button className="button danger-outline" disabled={busy} onClick={() => mutate(() => api(`/api/bible/entries/${draft.id}/archive`, { method: 'POST', body: JSON.stringify({ base_revision: draft.revision }) }), `“${draft.name}”已归档`)}>确认归档</button></div> : <button className="text-action danger-text" onClick={() => setArchiveConfirm(true)}><Archive size={14} />归档条目</button>)}</footer>
